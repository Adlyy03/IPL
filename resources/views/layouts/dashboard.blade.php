...
<body class="h-full flex text-slate-800 bg-slate-50 selection:bg-indigo-500 selection:text-white" x-data="{ sidebarOpen: false }">
                ...
                <div>
                    ...
                    <div class="space-y-1">
                        ...
                        <a href="{{ route('warga.profil.show') }}"
                           class="flex items-center px-3 py-2.5 text-xs font-semibold rounded-xl transition duration-150 {{ request()->routeIs('warga.profil.*') ? 'bg-indigo-600 text-white shadow-sm' : 'text-slate-300 hover:bg-slate-800/80 hover:text-white' }}">
                            <svg class="w-4 h-4 mr-3 {{ request()->routeIs('warga.profil.*') ? 'text-white' : 'text-slate-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header class="h-16 bg-white/95 backdrop-blur-md border-b border-slate-200/90 sticky top-0 z-30 flex items-center justify-between px-4 sm:px-6 lg:px-8">
            ...
            <!-- Topbar Right: Notification Bell + Role Pill + User Info + Logout -->
                ...
                @if (auth()->user()->isWarga())
                    ...
                    <div class="relative" x-data="{ notifOpen: false }" @click.outside="notifOpen = false">
                        ...
                        <button @click="notifOpen = !notifOpen"
                                type="button"
                                aria-label="Notifikasi Iuran"
                                class="relative p-2 rounded-xl text-slate-600 hover:text-slate-900 hover:bg-slate-100 border border-slate-200/90 transition focus:outline-none">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                            </svg>

                            @if ($unreadCount > 0)
                                <span class="absolute -top-1 -right-1 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-rose-600 px-1 text-[9px] font-bold text-white shadow-xs animate-pulse">
                        ...
                        <div x-show="notifOpen"
                             ...
                             class="absolute right-0 mt-2 w-80 sm:w-96 rounded-2xl bg-white shadow-xl border border-slate-200/90 py-2 z-50 overflow-hidden">
                            
                            <!-- Dropdown Header -->
                            <div class="px-4 py-2.5 border-b border-slate-100 flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    <span class="text-xs font-bold text-slate-900">Notifikasi Tagihan</span>
                                    @if ($unreadCount > 0)
                                        <span class="px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-rose-50 text-rose-600 border border-rose-200">
                                            {{ $unreadCount }} baru
                                        </span>
                                    @endif
                                </div>
                                ...
                                @if ($unreadCount > 0)
                                    <form method="POST" action="{{ route('warga.notifikasi.baca-semua') }}">
                                        @csrf
                                        <button type="submit" class="text-[11px] font-medium text-indigo-600 hover:text-indigo-800 transition">
                                            Tandai semua dibaca
                                        </button>
                                    </form>
                                @endif
                            ...
                            <!-- Dropdown Items -->
                            <div class="max-h-72 overflow-y-auto divide-y divide-slate-50 custom-scrollbar">
                                @forelse ($recentNotifications as $notif)
                                    <div class="p-3 hover:bg-slate-50/80 transition flex items-start justify-between gap-2.5 {{ is_null($notif->read_at) ? 'bg-indigo-50/30' : '' }}">
                                        <div class="flex items-start gap-2.5 min-w-0">
                                            <span class="w-2 h-2 rounded-full mt-1.5 shrink-0 {{ is_null($notif->read_at) ? 'bg-indigo-600' : 'bg-transparent' }}"></span>
                                            <div class="min-w-0">
                                                <p class="text-xs font-bold text-slate-900 truncate">
                                                    {{ $notif->data['title'] ?? 'Pemberitahuan Iuran' }}
                                                </p>
                                                <p class="text-[11px] text-slate-600 mt-0.5 leading-snug line-clamp-2">
                                                    {{ $notif->data['message'] ?? '-' }}
                                                </p>
                                                <span class="text-[10px] text-slate-400 mt-1 block font-mono">
                                        ...
                                        @if (is_null($notif->read_at))
                                            <form method="POST" action="{{ route('warga.notifikasi.baca', $notif->id) }}" class="shrink-0">
                                                @csrf
                                                <button type="submit" title="Tandai dibaca & buka" class="p-1 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-white transition">
                                ...
                                @empty
                                    <div class="py-8 text-center px-4">
                                        <div class="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center mx-auto text-slate-400 mb-2">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20 13V6a2 2 0 00-2-2H6a2 2 0 00-2 2v7m16 0v5a2 2 0 01-2 2H6a2 2 0 01-2-2v-5m16 0h-2.586a1 1 0 00-.707.293l-2.414 2.414a1 1 0 01-.707.293h-3.172a1 1 0 01-.707-.293l-2.414-2.414A1 1 0 006.586 13H4" />
                                            </svg>
                                        </div>
                                        <p class="text-xs font-medium text-slate-500">Belum ada notifikasi baru</p>
                                    </div>
                                @endforelse
                            </div>

                            <!-- Dropdown Footer -->
                            <div class="px-4 py-2 bg-slate-50/70 border-t border-slate-100 text-center">
                                <a href="{{ route('warga.iuran.index') }}" class="text-xs font-semibold text-indigo-600 hover:text-indigo-800">
                                    Buka Riwayat Tagihan &rarr;
                                </a>
                            </div>
                        </div>
                    </div>
