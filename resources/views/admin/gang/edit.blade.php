...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-slate-900">Edit Data Gang</h1>
            <p class="text-xs text-slate-500 mt-0.5">Perbarui informasi gang {{ $gang->nama_gang }}.</p>
        </div>
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <form action="{{ route('admin.gang.update', $gang) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="3"
                          class="w-full text-xs rounded-xl border-slate-300 py-2.5 px-3 focus:ring-indigo-500 focus:border-indigo-500 @error('keterangan') border-rose-300 ring-rose-200 @enderror">{{ old('keterangan', $gang->keterangan) }}</textarea>
                @error('keterangan')
                    <p class="text-rose-600 text-[11px] mt-1">{{ $message }}</p>
                @enderror
            </div>
