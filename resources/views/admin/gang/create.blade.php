...
@section('title', 'Tambah Gang Baru')
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-xl sm:text-2xl font-extrabold text-slate-900 tracking-tight">Tambah Gang Baru</h2>
            <p class="text-xs sm:text-sm text-slate-500 mt-1">Lengkapi formulir berikut untuk mendaftarkan gang atau jalan baru.</p>
        </div>
    ...
    <div class="bg-white p-6 sm:p-8 rounded-2xl sm:rounded-3xl border border-slate-200/90 shadow-xs">
        <form action="{{ route('admin.gang.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <input type="text" name="nama_gang" id="nama_gang" value="{{ old('nama_gang') }}" required placeholder="Contoh: Gang Kenanga / Jalan Mawar"
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="3" placeholder="Informasi tambahan letak jalan, patokan, atau catatan area..."
                          class="w-full text-xs sm:text-sm rounded-xl border-slate-200 py-2.5 px-3.5 focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition @error('keterangan') border-rose-300 ring-2 ring-rose-200 @enderror">{{ old('keterangan') }}</textarea>
                @error('keterangan')
                    <p class="text-rose-600 text-xs mt-1.5 flex items-center gap-1 font-medium">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <select name="status" id="status" required
                    ...
                    <option value="aktif" {{ old('status', 'aktif') === 'aktif' ? 'selected' : '' }}>Aktif (Dapat digunakan untuk blok/rumah)</option>
                    <option value="nonaktif" {{ old('status') === 'nonaktif' ? 'selected' : '' }}>Nonaktif (Tidak aktif sementara)</option>
                </select>
                @error('status')
                    <p class="text-rose-600 text-xs mt-1.5 flex items-center gap-1 font-medium">{{ $message }}</p>
                @enderror
