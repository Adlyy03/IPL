...
@section('page_heading', 'Input Tagihan Iuran Manual')

@section('content')
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-slate-900">Tambah Tagihan Iuran Manual</h1>
            <p class="text-xs text-slate-500 mt-0.5">Input tagihan individu atau pencatatan langsung untuk warga tertentu.</p>
        </div>
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <form action="{{ route('admin.iuran-warga.store') }}" method="POST" class="space-y-4" x-data="{ status: '{{ old('status_pembayaran', 'menunggu_pembayaran') }}' }">
            @csrf

            <!-- Pilih Warga -->
            <div>
                <label for="warga_id" class="block text-xs font-bold text-slate-700 mb-1.5">
                    Pilih Warga <span class="text-rose-500">*</span>
                </label>
                <select name="warga_id" id="warga_id" required
                    ...
                    @foreach ($wargas as $warga)
                        <option value="{{ $warga->id }}" {{ old('warga_id', request('warga_id')) == $warga->id ? 'selected' : '' }}>
                ...
                <select name="jenis_iuran_id" id="jenis_iuran_id" required
                    ...
                    @foreach ($jenisIurans as $jenis)
                        <option value="{{ $jenis->id }}" {{ old('jenis_iuran_id', request('jenis_iuran_id')) == $jenis->id ? 'selected' : '' }}>
                            {{ $jenis->nama_iuran }} (Tarif: Rp {{ number_format($jenis->nominal, 0, ',', '.') }})
                        </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="periode" id="periode" value="{{ old('periode', date('Y-m')) }}" required placeholder="Contoh: 2026-09"
            ...
            <div>
                ...
                <select name="status_pembayaran" id="status_pembayaran" required x-model="status"
                    ...
                    <option value="menunggu_pembayaran">Menunggu Pembayaran (Belum Lunas)</option>
            ...
            <div x-show="status === 'lunas'" class="space-y-4 pt-2 border-t border-slate-100" style="display: none;">
                <div>
                    <label for="tanggal_pembayaran" class="block text-xs font-bold text-slate-700 mb-1.5">
                        Tanggal & Waktu Pembayaran
                    </label>
                    <input type="datetime-local" name="tanggal_pembayaran" id="tanggal_pembayaran" value="{{ old('tanggal_pembayaran', date('Y-m-d\TH:i')) }}"
                           class="w-full text-xs rounded-xl border-slate-300 py-2.5 px-3 focus:ring-indigo-500 focus:border-indigo-500">
            ...
            <div>
                ...
                <textarea name="catatan" id="catatan" rows="2" placeholder="Catatan opsional..."
                          class="w-full text-xs rounded-xl border-slate-300 py-2.5 px-3 focus:ring-indigo-500 focus:border-indigo-500">{{ old('catatan') }}</textarea>
                @error('catatan')
                    <p class="text-rose-600 text-[11px] mt-1">{{ $message }}</p>
                @enderror
            </div>
