...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <div class="flex items-center gap-2">
                ...
                <h1 class="text-xl font-bold text-slate-900 tracking-tight">Tambah Jenis Iuran</h1>
            </div>
            <p class="text-xs text-slate-500 mt-1 pl-10">Definisikan kategori retribusi atau jenis iuran baru di lingkungan warga.</p>
        </div>
    ...
    <div class="bg-white p-6 sm:p-7 rounded-2xl border border-slate-200/90 shadow-xs">
        <form action="{{ route('admin.jenis-iuran.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div>
                ...
                <textarea name="deskripsi" id="deskripsi" rows="3" placeholder="Jelaskan peruntukan pemanfaatan iuran ini untuk warga..."
                          class="w-full text-xs rounded-xl border border-slate-300 py-2.5 px-3 text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-600 transition @error('deskripsi') border-rose-300 ring-2 ring-rose-100 @enderror">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="text-rose-600 text-[11px] mt-1.5 flex items-center gap-1 font-medium">
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                        {{ $message }}
