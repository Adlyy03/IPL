...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-slate-900">Edit Blok {{ $blok->nama_blok }} No. {{ $blok->nomor_rumah }}</h1>
            <p class="text-xs text-slate-500 mt-0.5">Perbarui informasi blok dan nomor rumah.</p>
        </div>
    ...
    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <form action="{{ route('admin.blok.update', $blok) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="gang_id" id="gang_id" required
                        class="w-full text-xs rounded-xl border-slate-300 py-2.5 px-3 focus:ring-indigo-500 focus:border-indigo-500 @error('gang_id') border-rose-300 ring-rose-200 @enderror">
            ...
            <div>
                ...
                <input type="text" name="nama_blok" id="nama_blok" value="{{ old('nama_blok', $blok->nama_blok) }}" required
                       class="w-full text-xs rounded-xl border-slate-300 py-2.5 px-3 focus:ring-indigo-500 focus:border-indigo-500 @error('nama_blok') border-rose-300 ring-rose-200 @enderror">
                @error('nama_blok')
                    <p class="text-rose-600 text-[11px] mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" name="nomor_rumah" id="nomor_rumah" value="{{ old('nomor_rumah', $blok->nomor_rumah) }}" required
                       class="w-full text-xs rounded-xl border-slate-300 py-2.5 px-3 focus:ring-indigo-500 focus:border-indigo-500 @error('nomor_rumah') border-rose-300 ring-rose-200 @enderror">
                @error('nomor_rumah')
                    <p class="text-rose-600 text-[11px] mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="2"
                          class="w-full text-xs rounded-xl border-slate-300 py-2.5 px-3 focus:ring-indigo-500 focus:border-indigo-500 @error('keterangan') border-rose-300 ring-rose-200 @enderror">{{ old('keterangan', $blok->keterangan) }}</textarea>
                @error('keterangan')
                    <p class="text-rose-600 text-[11px] mt-1">{{ $message }}</p>
                @enderror
            </div>
